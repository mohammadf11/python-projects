...
class TestArticleListCreateAPIView(BaseTest):
    def setUp(self) -> None:
        super().setUp()
        self.url = reverse('blog:api_article-list')
    ...
    def test_creat(self):
        pass  # test in permission class


class TestRetrieveUpdateDestroyAPIView(BaseTest):
    def setUp(self) -> None:
        super().setUp()
        self.id = 1
        self.url = reverse('blog:api_article-detail', args=[self.id])

    def test_retrieve(self):
        response = self.client.get(self.url)
        self.assertEqual(response.data['title'],
                         Article.objects.get(id=self.id).title)
    ...
    def test_update(self):
        ...
        self.assertEqual(Article.objects.get(
            id=self.id).title, 'update article')
